    ...
    namespace data {
        ...
        class TypedIterator {
          ...
          private:
            ...
            template <typename U>
            typename std::enable_if<!std::is_arithmetic<U>::value && !is_complex<U>::value, reference>::type getRef() const MW_NOEXCEPT {
            ...
            template <typename U, bool is_const>
            typename std::enable_if<!std::is_arithmetic<U>::value && !is_complex<U>::value && !std::is_same<Struct, U>::value && is_const, ref_type>::type createTempRef(std::shared_ptr<detail::IteratorImpl> impl) MW_NOEXCEPT {
                pImpl = std::move(impl);
                return ref_type();
            }
